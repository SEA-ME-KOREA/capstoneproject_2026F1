...
본 시스템은 **ROS2 + Gazebo** 기반의 **원격 주차 시뮬레이션 시스템**이다. 이중주차(double parking)된 사용자 차량(LIMO1)이 상대 차량(LIMO2/A2/A4)의 출차를 지원한 뒤, 비워진 슬롯에 자동으로 재주차하는 전 과정을 자율적으로 수행한다.
...
| `my_valet_parking` | LIMO 개별 제어 노드 모음 (발렛 주차, 출차, 슬롯 추적 등) |
| `remote_parking_world` | Gazebo 월드 파일, URDF/XACRO, launch 파일 |
...
```
          북쪽 벽 (y=1.585)
   ┌──────────────────────────────┐
   │  [A1]   [A2]   [A3]   [A4]  │  ← 주차 슬롯 (y=0.985)
   │ x=-0.675 -0.225 0.225 0.675  │
   │                              │
   │        [LIMO1]               │  ← 이중주차 위치 (x=0.225, y=0.35, yaw=0)
   │                              │     차로 위에 A3 앞을 막고 있음
   │──────── 차로 (y≈0) ──────────│
   │                              │
   └──────────────────────────────┘
...
- **LIMO1**: (0.225, 0.35) yaw=0 - 차로에서 A3 슬롯 앞을 막고 있는 이중주차 상태
- **LIMO2**: (0.225, 0.985) yaw=-π/2 - A3 슬롯에 주차된 상태
- **LIMO A2**: (-0.225, 0.985) yaw=-π/2 - A2 슬롯에 주차된 상태
- **LIMO A4**: (0.675, 0.985) yaw=-π/2 - A4 슬롯에 주차된 상태
...
| RGB 카메라 | `rgb/image_raw` | 비전 정렬(템플릿 매칭), 차선 인식 |
...
[mission_manager] ──pub──→ /limo1/cmd_vel ──→ LIMO1 구동
                  ...
                  ←─sub── /select_exit_car (from user script)
...
1. **world_server.launch.py** (1회 실행) - Gazebo 물리 엔진 + 정적 월드 로드
2. **spawn_robots.launch.py** (테스트마다 실행):
   - 0s: robot_state_publisher x4 시작
   - 3s: Gazebo 로봇 스폰 x4
...
#### 목적
A3 슬롯 앞을 막고 있는 LIMO1을 후진시켜 상대 차량의 출차 경로를 확보한다.
...
2. 매 제어 주기 (50ms):
   ...
   b. 목표 도달 체크:
      - 현재 위치와 evade_goal(-0.5, 0.35) 사이 거리 < 0.04m이면 완료
...
#### 작동 방식
- `/select_exit_car` 토픽으로 문자열 메시지 수신 (a2, a3, a4 중 하나)
- 별도 터미널에서 `bash scripts/select_car.sh a3` 실행
- 유효한 선택이 들어오면 → `LIMO2_EXIT_INIT`으로 전환
- 선택된 차량에 따라 `repark_goal`이 동적으로 업데이트됨 (출차한 슬롯이 재주차 목표)
...
mission_manager는 `/limo1/target_slot` 수신 시 → `LIMO1_REPARK` 전환. 10초 타임아웃 시 파라미터 fallback 좌표 사용.
...
- **2D LiDAR** (`/limo1/scan`): (간접) 장애물 맵 구축용
...
1. Occupancy Grid 구축:
   - 범위: x[-1.1, 1.2], y[-0.6, 1.7], 해상도 0.02m
   - 주차된 차량 4대를 장애물로 등록 (출차한 슬롯은 제외)
   - 북벽, 서벽을 장애물로 등록
...
5. Reeds-Shepp 곡선 가지치기:
   ...
   - 6가지 기본 공식(LSL, LSR, LRL) × 4 변환 = 24가지 RS 경로 유형
...
(1) LIMO1_EVADE: 후진 이중주차 해제
    ...
    종료조건: 목표 도달 OR 1.5초 이상 후방 차단
    │
    ▼
(2) WAIT_FOR_SELECTION: 출차 차량 선택
    입력: /select_exit_car 토픽
    │
    ▼
...
사용자가 `select_car.sh` 스크립트로 a2/a3/a4 중 하나를 선택하면, mission_manager가 해당 차량의 odom과 cmd_vel 퍼블리셔를 동적으로 바인딩한다.
...
5. /limo2/cmd_vel (또는 선택된 차량) 으로 Twist 발행
```
...
| 재주차 경로 계획 | Hybrid A* + Reeds-Shepp 곡선 (24가지 유형) | Odometry (grid는 정적) |
...
2. **다중 차량 지원**: A2/A3/A4 중 사용자 선택에 따라 동적으로 출차 대상 변경
